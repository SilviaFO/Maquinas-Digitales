...
void setup() {
 ...
 if (!bmp.begin(
	...
	while (1) {}
  } //Inicializar comunicación I2C con el sensor barométrico 
  ...
  //*****************************
  // Cert leer archivo
  File file4 = SPIFFS.open("/8edf-certificate.pem.crt", FILE_READ);
  if (!file4) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  File file6 = SPIFFS.open("/8edf-private.pem.key", FILE_READ);
  if (!file6) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
